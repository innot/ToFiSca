...
from mock_picamera2 import formats
...
class Request:
    def __init__(self, camera: Camera, cookie: str):
        self.camera = camera
        self._cookie = cookie
        self._status = RequestStatus.RequestPending
        ...
        with self.lock:
            ...
            if self.ref_count < 0:
                ...
                if self.picam2.camera and self.stop_count == self.picam2.stop_count and self.picam2.started:
                    self.request.reuse()
                    controls = self.picam2._controls.get_libcamera_controls()
                    for id, value in controls.items():
                        self.request.set_control(id, value)
                    self.picam2._controls = Controls(self.picam2)
    ...
    def save(self, img, metadata, file_output, format=None, exif_data=None):
        ...
        if format_str in ('jpg', 'jpeg'):
            ...
            if "AnalogueGain" in metadata and "DigitalGain" in metadata:
                ...
                zero_ifd = {piexif.ImageIFD.Make: "Mock Picamera2",
